...
class Building:
    ...
    def fire_alarm(self):
        for ele in self.elevators:
            if ele.current_floor != self.bottom_floor:
                print(f"\nThe elevator {ele.elevator_id} is on {ele.current_floor} floor, return to the bottom floor immediately.")
                ele.go_to_floor(self.bottom_floor)
            else:
                print(f"\nThe elevator {ele.elevator_id} was on the bottom floor.")
